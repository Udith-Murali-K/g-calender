...
export class HomePage {
  calendarEvent:any = {};
  constructor(public navCtrl: NavController,private iab: InAppBrowser,private browserTab: BrowserTab) {
  ...
  CLIENT_ID = '648691753664-6jig2qskdi128js347e6qeqd3imhi8mv.apps.googleusercontent.com';
  SCOPES = ["https://www.googleapis.com/auth/calendar"];
  APIKEY = "AIzaSyBZqRz561SgxozHvMuf-RJ5TBvcGNDnMtU";
  ...
  sendInvite() {
    ...
    this.browserTab.isAvailable()
        .then((isAvailable: boolean) => {
          ...
          if (isAvailable) {
            ...
            this.browserTab.openUrl('https://accounts.google.com/o/oauth2/auth?client_id=' + this.CLIENT_ID + '&redirect_uri=' + this.REDIRECTURL + '&scope=https://www.googleapis.com/auth/calendar&approval_prompt=force&response_type=token');
    ...
    // var browserRef = this.iab.create('https://accounts.google.com/o/oauth2/auth?client_id=' + this.CLIENT_ID + '&redirect_uri=' + this.REDIRECTURL + '&scope=https://www.googleapis.com/auth/calendar&approval_prompt=force&response_type=token', '_blank', 'location=no');
    // // browserRef.addEventListener("loadstart", (event) => {
    // //
    // // });
    // browserRef.on("loadstart").subscribe(res=>{
    //   if ((res["url"]).indexOf("http://localhost/callback") === 0) {
    //     var url = res["url"];
    //     var token = url.split('access_token=')[1].split('&token_type')[0];
    //     browserRef.on("exit").subscribe(res=>{
    //
    //     });
    //     browserRef.close();
    //
